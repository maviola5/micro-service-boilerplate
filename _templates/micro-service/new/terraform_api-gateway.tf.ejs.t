---
to: terraform/api-gateway.tf
---
resource "aws_api_gateway_rest_api" "api" {
  <%= name.toLowerCase() %>        = local.application
...
resource "aws_lambda_permission" "apigw_lambda" {
  ...
  function_<%= name.toLowerCase() %> = aws_lambda_function.function.function_<%= name.toLowerCase() %>
...
resource "aws_api_gateway_deployment" "api" {
  ...
  rest_api_id = aws_api_gateway_rest_api.api.id
  stage_<%= name.toLowerCase() %>  = local.application
}
